...
for in_feat in feats:
    ...
    part_idx = 0
    print(len(polygons), 'parts')
    for polygon in polygons:
        print('===================== part', part_idx, '-', len(polygon), 'rings =====================')
        if len(polygon) > 100:
            print('fid:', in_feat.id(), 'rings:', len(polygon))
        ring_idx = 0
        for ring in polygon:
            print('--------------------- ring', ring_idx, '-', len(ring), 'vertices ---------------------')
            vertex_cnt = len(ring)
            ...
            for vertex in ring:
                if vertex[0] <= -180 or vertex[0] >= 180 or vertex[1] <= -90 or vertex[1] >= 90:
                    print('!!! ---> ERROR: ', 'fid:', in_feat.id(), 'out of bounds:', vertex)
                #print(vertex[0], vertex[1])
            ring_idx += 1
        part_idx += 1
    if not in_geom.isGeosValid ():
       print('!!! ---> ERROR: ', 'fid:', in_feat.id(), 'NOT geos valid')
       geos_invalid_cnt += 1
       continue
